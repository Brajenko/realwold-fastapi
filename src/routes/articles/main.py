...
async def list_articles(
    ...
    db_author = get_user_by_username(db, author) if author is not None else None
    db_favorited = get_user_by_username(db, favorited) if favorited is not None else None
    ...
    return schemas.MultipleArticlesResponse(articles=articles, articlesCount=len(articles))
...
async def get_article(
    db_article: models.Article = Depends(get_article_dependency),
    db: Session = Depends(get_db),
    user: USER = Depends(get_user_optional),
) -> schemas.ArticleResponse:
    return schemas.ArticleResponse(article=controller.article_to_schema(db_article, user))
